...
export class ComparisonService {
  ...
  private async getEntityMetrics(
    ...
    try {
      ...
      const metrics: ComparisonMetrics = {
        ...
        salaryRange: `$${Math.round(data.min_salary || 0)}K - $${Math.round((data.max_salary || 0) / 1000)}K`,
  ...
  private buildEntityWhereClause(entity: ComparisonEntity): string {
    ...
    // TODO: Add timeframe filter when properly implemented
  ...
  private calculateRankings(entities: ComparisonEntityWithMetrics[]): RankingData[] {
    ...
    for (const metric of metrics) {
      ...
      // TODO: Update entity rank data - needs proper interface implementation
      // sortedEntities.forEach((entity, index) => {
  ...
  private generateMarketInsights(entities: ComparisonEntityWithMetrics[]): string[] {
    const insights: string[] = [];
    
    // Find salary outliers
    const avgSalaries = entities.map(e => e.metrics.avgSalary as number);
    const salaryMean = avgSalaries.reduce((sum, sal) => sum + sal, 0) / avgSalaries.length;
    
    for (const entity of entities) {
      if ((entity.metrics.avgSalary as number) > salaryMean * 1.2) {
