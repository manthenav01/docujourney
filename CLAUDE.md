...
## Caching & the ISR write budget

ISR writes are the binding free-tier constraint on this project, not BigQuery or bandwidth.
...
- Freshness comes from `POST /api/revalidate` (bearer `REVALIDATE_SECRET`), which `scripts/data_pipeline.py` calls after it rebuilds the serving aggregates.
