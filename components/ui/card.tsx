...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-card text-card-foreground flex flex-col gap-4 rounded-2xl border border-border/60 py-6 shadow-sm backdrop-blur-sm transition-all duration-200 hover:shadow-md hover:border-border/80',
        className,
      )}
      {...props}
    />
